...
trait DefaultJsonFormats extends DefaultJsonProtocol with SprayJsonSupport with MetaMarshallers {
  ...
  implicit object DateJsonFormat extends RootJsonFormat[Date] {
    ...
    def read(value: JsValue) = value match {
      case JsString(x) => DateFormat.getDateInstance(DateFormat.SHORT).parse(x)
  ...
  implicit object PointJsonFormat extends RootJsonFormat[Point] {
    def write(x : Point) = JsObject(
          "x" -> JsString(x.x), 
          "y" -> JsArray(x.y.map(_.toJson).toList), 
          "tooltip" -> JsString(x.tooltip))
    def read(value : JsValue) = value.asJsObject.getFields("x", "y", "tooltip") match {
      case Seq(JsString(x), JsArray(y), JsString(t)) => new Point(x, y.map(_.convertTo[Int]).toList, t)
    }
